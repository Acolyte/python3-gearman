...
from python3_gearman.protocol import GEARMAN_PARAMS_FOR_COMMAND, \
    ...
    parse_text_command, pack_text_command
...
class GearmanConnection(object):
    ...
    Wraps a socket and provides the following functionality:
        ...
        Full read/write methods for Gearman SERVER commands and responses
        (using GEARMAN_COMMAND_TEXT_COMMAND)
    ...
    def writable(self):
        ...
        return self.connected and bool(
            self._outgoing_commands or self._outgoing_buffer)
    ...
    def read_commands_from_buffer(self):
        ...
        while True:
            ...
            # Move the self._incoming_buffer forward by the number of bytes we
            # just read
    ...
    def _unpack_command(self, given_buffer):
        """Conditionally unpack a binary command or a text based server
        command"""
    ...
    def _pack_command(self, cmd_type, cmd_args):
        ...
        if _DEBUG_MODE_:
            gearman_logger.debug('%s - Send - %s - %r',
                                 hex(id(self)),
                                 get_command_name(cmd_type),
                                 cmd_args)
    ...
    def throw_exception(self, message=None, exception=None):
        ...
        # Allows catchers of ConnectionError a chance to inspect the last state
        # of this connection
        self.connected = False
        ...
        rewritten_message = "<%s:%d> %s" % (
            self.gearman_host, self.gearman_port, message)
